...
Buffer::Buffer(std::string &filename) : filename_(filename), is_modified(false) {
    ...
    for (size_t i = 0; i < size; ++i) {
        char byte = 0;
        if (file.read(reinterpret_cast<char*>(&byte), 1)) {data_.push_back(byte);}
    }
    file.close();
}
...
std::deque<unsigned char>& Buffer::get_data() {return data_;}
...
int Buffer::save() {
    ...
    for (size_t i = 0; i < data_.size(); ++i) {
        if (!file.write(reinterpret_cast<char*>(&data_[i]), 1)) {return 3;}
    }
    file.close();
    is_modified = false;
    return 0;
...
int Buffer::save(std::string &filename) {
    if (!filename_.empty()) {return 1;}
    filename_ = filename;
    return save();
}

int Buffer::insert_byte(const unsigned char byte, const size_t &index) {
